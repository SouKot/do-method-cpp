...
using namespace Teuchos;
    ...
    if (output)
      ...
      filename_=params.get("File Name","Trilinos_LocaInterface");
...
void OceanOutputXdmf::addAttribute(XMLObject& state, string name, 
  ...
  string dimstring = toString(L)+" "+toString(M)+" "+toString(N);
...
void OceanOutputXdmf::FillXmlGrid(XMLObject& grid,
            string gridname, string groupname,
...
void OceanOutputXdmf::WriteXmf(XMLObject& grid, string filename)
  {
  if (comm_->MyPID()==0)
    {
...
void OceanOutputXdmf::WriteXmf(XMLObject& grid1,XMLObject& grid2, string filename)
  {
  if (comm_->MyPID()==0)
    {
    std::ofstream xmlfile(filename.c_str());
