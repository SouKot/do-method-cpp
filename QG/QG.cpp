...
void
QG::jacob(double const* un, double sig)
{
  Vector2D om(n_, m_);
  Vector2D ps(n_, m_);
...
void
QG::jacobian(double const* un, double sig, int* beg, int* jco, double* co)
{
  Vector2D om(n_, m_);
  Vector2D ps(n_, m_);
  u_to_psi(un, om, ps);
...
void
QG::writeA(char const* name, double const* un, double sig)
{
  jacob(un, sig);
...
void
QG::rhs(double const* un, double* b)
{
  Vector2D om(n_, m_);
  Vector2D ps(n_, m_);
  Vector1D Frc(ndim_);
...
void
QG::bilin(double const* un, double const* vn, double* b)
{
  Vector2D om(n_, m_);
  Vector2D ps(n_, m_);
...
void
QG::apply(double const* x, double* y)
{
  for (int i = 0; i < ndim_; i++) {
    y[i] = 0.0;
    for (int v = A_.beg[i]; v < A_.beg[i + 1]; v++)
...
void
QG::nlin_rhs(Vector2D const& om, Vector2D const& ps)
{
...
void
QG::nlin_jac(Vector2D const& om, Vector2D const& ps)
{
  ...
  for (int i = 0; i < Nlzz_.size(); i++)
    Nlzz_[i] = Udx[i] + Vdy[i];
  for (int i = 0; i < Nlzp_.size(); i++)
    // Nlzp_[i] = udZx[i] + vdZy[i] - F*(Udx[i] + Vdy[i]) - F*(udPx[i] +
    // vdPy[i]);
...
void
QG::u_to_psi(double const* un, Vector2D& om, Vector2D& ps)
{
  for (int j = 0; j < m_; j++) {
    for (int i = 0; i < n_; i++) {
      int row = 2 * (n_ * j + i);
...
QG::boundaries()
{
  ...
  double oml3 =
    -3 * par_(6) / (dx_ * dx_); // for equidistant grids no differences
 ...
 */
void
QG::nonlin(int type, Vector3D& atom, Vector2D const& om, Vector2D const& ps)
{
  /*
...
void
QG::deriv(int type, Vector3D& atom)
{
  /*
   *     1:  z, p
   *     2:  zxx, pxx
