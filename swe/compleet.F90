...
!*
!* ============================================================================ *
      SUBROUTINE DSM(M,N,NPAIRS,INDROW,INDCOL,NGRP,MAXGRP,MINGRP, INFO,IPNTR,JPNTR,IWA,LIWA)
      INTEGER M,N,NPAIRS,MAXGRP,MINGRP,INFO,LIWA
      INTEGER INDROW(NPAIRS),INDCOL(NPAIRS),NGRP(N), IPNTR(M+1),JPNTR(N+1),IWA(LIWA)
!C     **********
!C
!C     SUBROUTINE DSM
!C
...
!C
      INFO = 0
      IF (M .LT. 1 .OR. N .LT. 1 .OR. NPAIRS .LT. 1 .OR. LIWA .LT. MAX(M,6*N)) RETURN
      DO 10 K = 1, NPAIRS
         INFO = -K
         IF (INDROW(K) .LT. 1 .OR. INDROW(K) .GT. M .OR. INDCOL(K) .LT. 1 .OR. INDCOL(K) .GT. N) RETURN
   10    CONTINUE
      INFO = 1
!C
...
!C
      CALL SLO(N,INDROW,JPNTR,INDCOL,IPNTR,IWA(5*N+1),IWA(4*N+1), MAXCLQ,IWA(1),IWA(N+1),IWA(2*N+1),IWA(3*N+1))
...
!C
      CALL IDO(M,N,INDROW,JPNTR,INDCOL,IPNTR,IWA(5*N+1),IWA(4*N+1), MAXCLQ,IWA(1),IWA(N+1),IWA(2*N+1),IWA(3*N+1))
...
!C
      END
      SUBROUTINE IDO(M,N,INDROW,JPNTR,INDCOL,IPNTR,NDEG,LIST,MAXCLQ,IWA1,IWA2,IWA3,IWA4)
      INTEGER M,N,MAXCLQ
      INTEGER INDROW(*),JPNTR(N+1),INDCOL(*),IPNTR(M+1),NDEG(N), LIST(N),IWA1(0:N-1),IWA2(N),IWA3(N),IWA4(N)
!C     **********
!C
!C     SUBROUTINE IDO
!C
...
!C
      ...
      INTEGER N,MAXGRP
      INTEGER INDROW(*),JPNTR(N+1),INDCOL(*),IPNTR(*),LIST(N),NGRP(N),IWA(N)
!C     **********
!C
!C     SUBROUTINE SEQ
!C
...
!C
      END
      SUBROUTINE SLO(N,INDROW,JPNTR,INDCOL,IPNTR,NDEG,LIST,MAXCLQ,IWA1,IWA2,IWA3,IWA4)
      INTEGER N,MAXCLQ
      INTEGER INDROW(*),JPNTR(N+1),INDCOL(*),IPNTR(*),NDEG(N),LIST(N),IWA1(0:N-1),IWA2(N),IWA3(N),IWA4(N)
!C     **********
!C
!C     SUBROUTINE SLO
!C
...
!C        FOUND DURING THE ORDERING.
!C
         IF (MINDEG + 1 .EQ. NUMORD .AND. MAXCLQ .EQ. 0) MAXCLQ = NUMORD
!C
...
!c  M,N,COL,IND,NPNTR,NGRP,NUMGRP,D,FJACD,FJAC
!C
      IF (COL) THEN
!C
...
!C
         DO 50 IROW = 1, M
            DO 30 IP = NPNTR(IROW), NPNTR(IROW+1)-1
               JCOL = IND(IP)
               IF (NGRP(JCOL) .EQ. NUMGRP) THEN
!c	          write (81,'(3i5, es16.8)') irow, ip, jcol, d(jcol)
